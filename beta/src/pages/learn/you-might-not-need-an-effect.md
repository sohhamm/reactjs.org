...
function Game() {
  ...
  function handlePlaceCard(nextCard) {
    ...
    // ✅ Adjust related state in the event handler
    setCard(nextCard);
    if (nextCard.gold) {
      if (goldCardCount <= 3) {
...
In some cases, you *can't* calculate the next state directly in the event handler. For example, imagine a form with multiple dropdowns where the options of each next dropdown depend on the selected value of the previous dropdown. Then, a chain of [Effects fetching data](/learn/you-might-not-need-an-effect#fetching-data) is appropriate because you are synchronizing with network.
